...
typedef char element;		// 이진 탐색 트리 elemnet의 자료형을 char로 정의
...
treeNode* searchBST(treeNode* root, char x) {
	...
	p = root;
	while (p != NULL) {
		if (x < p->key) p = p->left;	// x의 값이 p보다 작으면 왼쪽 트리로 이동
		else if (x == p->key) return p;	// x의 값이 같으면 p를 반환
		else p = p->right;
	}
	printf("\n 찾는 키가 없습니다!");
	return p;
...
void deleteNode(treeNode* root, element key) {
	...
	if ((p->left == NULL) && (p->right == NULL)) {
		if (parent != NULL) {
			if (parent->left == p) parent->left = NULL;	// 왼쪽 노드로 이동했을때 NULL
			else parent->right = NULL;					// 오른쪽 노드로 이동했을때 NULL
		}
		else root = NULL;
	}
