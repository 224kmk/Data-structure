...
// 서브 트리의 높이를 구하는 연산
int getHegiht(treeNode* p) {
	int height = 0;
	if (p != NULL) height = MAX(getHegiht(p->left), getHegiht(p->right)) + 1;
	return height;
}
...
int getBF(treeNode* p) {
	if (p == NULL) return 0;
	return getHegiht(p->left) - getHegiht(p->right);
}
...
// AVL 트리에 노드를 삽인하는 연산 : 이진 탐색 연산처럼 삽입한 후에, rebalance() 호출
...
treeNode* insert_BST_Node(treeNode* p, element x) {
	treeNode* newNode;
	if (p == NULL) {					// 포인터 p가 NULL 일경우 노드를 생성함
...
int main() {
	...
	insert_AVL_Node(&root_BST, 60);
	insert_AVL_Node(&root_BST, 70);
	insert_AVL_Node(&root_BST, 90);
	insert_AVL_Node(&root_BST, 80);
	insert_AVL_Node(&root_BST, 75);
	insert_AVL_Node(&root_BST, 73);
	insert_AVL_Node(&root_BST, 72);
	insert_AVL_Node(&root_BST, 78);
